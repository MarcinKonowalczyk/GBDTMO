...
BoosterMulti::BoosterMulti(const Shape s, HyperParameters hp) : BoosterBase(s, hp) {
    Score.resize(shape.out_dim);
    if (hp.topk > 0) {
        OptPair.resize(hp.topk);
    } else {
        Opt.resize(shape.out_dim);
    }
}
...
void BoosterMulti::boost_all(const std::vector<Histogram>& Hist) {
    ...
    int divisor = (hp.topk == 0) ? shape.out_dim : hp.topk;
    meta.reset();
    for (size_t i  = 0; i < shape.inp_dim; ++i) {
...
void BoosterMulti::build_tree_best() {
    ...
    if (rows_l >= rows_r) {
        for (size_t i = 0; i < shape.inp_dim; ++i) { Hist_r[i] = Histogram(bin_nums[i], shape.out_dim); }
        hist_all(order_r, Hist_r);
        for (size_t i = 0; i < shape.inp_dim; ++i) { info.hist[i] - Hist_r[i]; }
        Hist_l.assign(info.hist.begin(), info.hist.end());
    } else {
        for (size_t i = 0; i < shape.inp_dim; ++i) { Hist_l[i] = Histogram(bin_nums[i], shape.out_dim); }
    ...
    if (rows_l >= hp.min_samples) {
        if (hp.topk == 0) {
            get_score_opt(Hist_l[rand() % shape.inp_dim], Opt, Score, Score_sum);
        } else {
            get_score_opt(Hist_l[rand() % shape.inp_dim], OptPair, Score, Score_sum);
        }
        boost_all(Hist_l);
    ...
    if (rows_r >= hp.min_samples) {
        if (hp.topk == 0) {
            get_score_opt(Hist_r[rand() % shape.inp_dim], Opt, Score, Score_sum);
        } else {
            get_score_opt(Hist_r[rand() % shape.inp_dim], OptPair, Score, Score_sum);
        }
        boost_all(Hist_r);
        if (meta.is_set && depth + 1 < hp.max_depth && meta.gain > hp.gamma) {
...
void BoosterMulti::growth() {
    ...
    std::vector<Histogram> Hist(shape.inp_dim);
    for (size_t i = 0; i < shape.inp_dim; ++i) { Hist[i] = Histogram(bin_nums[i], shape.out_dim); }
    hist_all(Data.train_order, Hist);
    if (hp.topk == 0) {
        get_score_opt(Hist[rand() % shape.inp_dim], Opt, Score, Score_sum);
    } else {
        get_score_opt(Hist[rand() % shape.inp_dim], OptPair, Score, Score_sum);
    }
    boost_all(Hist);
...
void BoosterMulti::predict(
    ...
    num_trees = num_trees == 0 ? trees.size() : std::min(num_trees, trees.size());
    for (size_t i = 0; i < num_trees; ++i) { trees[i].pred_value_multi(features, preds, shape, n); }
}
